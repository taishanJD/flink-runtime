...
public class KMeansNode extends AbstractTaskNode {

    private static final Logger LOGGER = LoggerFactory.getLogger(KMeansNode.class);
    ...
    public DataSet calculate(DataSet in, List<String> inRecordKeys, List<String> outRecordKeys) {
        ...
        LOGGER.info("[K_MEANS] Start calculation, in= {}", in.toString());
        ...
        try {
            ...
            LOGGER.info("[K_MEANS] vectors= {}", vectors);
            if (vectors.size() == inputKeys.size()) { // 将聚类数据写入临时文件
                ...
                LOGGER.error("[K_MEANS] Feature value extraction from given data failed~!!");
            }
...
//            System.out.println("Preparing Input");
            LOGGER.info("[K_MEANS] Preparing Input");
            InputDriver.runJob(input, directoryContainingConvertedInput, "org.apache.mahout.math.RandomAccessSparseVector");
//            System.out.println("Running random seed to get initial clusters");
            LOGGER.info("[K_MEANS] Running random seed to get initial clusters");
            Path clusters = new Path(output, "random-seeds");
            clusters = RandomSeedGenerator.buildRandom(conf, directoryContainingConvertedInput, clusters, k, measure);
//            System.out.println("Running KMeans with k = " + k);
            LOGGER.info("[K_MEANS] Running Fuzzy KMeans with k: {}", k);
            LOGGER.info("[K_MEANS] Run Clustering: {}", runClustering);
            KMeansDriver.run(conf, directoryContainingConvertedInput, clusters, output, convergenceDelta, maxIterations, runClustering, 0.0D, false);
            Path outGlob = new Path(output, "clusters-*-final");
            Path clusteredPoints = new Path(output, "clusteredPoints");
//            System.out.println("Dumping out clusters from clusters: " + outGlob + " and clusteredPoints:" + clusteredPoints);
            LOGGER.info("[K_MEANS] Dumping out clusters from clusters: {} and clusteredPoints: {}", outGlob, clusteredPoints);
            ...
            LOGGER.info("[K_MEANS] resultValue==== {}", resultSb.toString());
        ...
        LOGGER.info("[K_MEANS] Returning calculation result");
        return out;
    }
