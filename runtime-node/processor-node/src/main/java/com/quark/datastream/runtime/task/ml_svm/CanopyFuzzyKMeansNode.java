...
public class CanopyFuzzyKMeansNode extends AbstractTaskNode {

    private static final Logger LOGGER = LoggerFactory.getLogger(CanopyFuzzyKMeansNode.class);
    ...
    public DataSet calculate(DataSet in, List<String> inRecordKeys, List<String> outRecordKeys) {
        ...
        LOGGER.info("[CANOPY_FUZZY_KMEANS] Start calculation, in= {}", in.toString());
        ...
        try {
            ...
            LOGGER.info("[CANOPY_FUZZY_KMEANS] vectors= {}", vectors);
            if (vectors.size() == inputKeys.size()) { // 将聚类数据写入临时文件
                ...
                LOGGER.error("[CANOPY_FUZZY_KMEANS] Feature value extraction from given data failed~!!");
            }
...
//            System.out.println("Preparing Input");
            LOGGER.info("[CANOPY_FUZZY_KMEANS] Preparing Input");
            InputDriver.runJob(input, directoryContainingConvertedInput, "org.apache.mahout.math.RandomAccessSparseVector");
//            System.out.println("Running Canopy to get initial clusters");
            LOGGER.info("[CANOPY_FUZZY_KMEANS] Running Canopy to get initial clusters");
            Path canopyOutput = new Path(output, "canopies");
            CanopyDriver.run(new Configuration(), directoryContainingConvertedInput, canopyOutput, measure, t1, t2, false, 0.0D, false);
//            System.out.println("Running Fuzzy KMeans with fuzziness: " + fuzziness);
            LOGGER.info("[CANOPY_FUZZY_KMEANS] Running Fuzzy KMeans with fuzziness: {}", fuzziness);
            LOGGER.info("[CANOPY_FUZZY_KMEANS] Run Clustering: {}", runClustering);
            FuzzyKMeansDriver.run(directoryContainingConvertedInput, new Path(canopyOutput, "clusters-0-final"), output, convergenceDelta, maxIterations, fuzziness, runClustering, true, 0.0D, false);
            Path outGlob = new Path(output, "clusters-*-final");
            Path clusteredPoints = new Path(output, "clusteredPoints");
//            System.out.println("Dumping out clusters from clusters: " + outGlob + " and clusteredPoints:" + clusteredPoints);
            LOGGER.info("[CANOPY_FUZZY_KMEANS] Dumping out clusters from clusters: {} and clusteredPoints: {}", outGlob, clusteredPoints);
            ...
            LOGGER.info("[CANOPY_FUZZY_KMEANS] resultValue==== {}", resultSb.toString());
        ...
        LOGGER.info("[CANOPY_FUZZY_KMEANS] Returning calculation result");
        return out;
    }
