...
public class RegressionNode extends AbstractTaskNode {
    private static final Logger LOGGER = LoggerFactory.getLogger(RegressionNode.class);
    ...
    public DataSet calculate(DataSet in, List<String> inRecordKeys, List<String> outRecordKeys) {
        LOGGER.info("[Regression] String calculation , in= {}", in.toString());
        ...
        LOGGER.info("[Regression] vectors= {}", vectors);
        if (vectors.size() == inputKeys.size()) {
            ...
            LOGGER.info("[Regression] resultValue==== {}", resultValue);
    ...
    public void setParam(TaskNodeParam params) {
        LOGGER.info("[Regression] Entering setParam Method");
        try {
            ...
            // EPSILON_SVR(3), NU_SVR(4)
            if (params.containsKey("svmType")) {
            ...
            LOGGER.error("[Regression] " + e.toString());
        } catch (NullPointerException e) {
            LOGGER.error("[Regression] " + e.toString());
        }

        LOGGER.info("[Regression] Leaving setParam Method");
    }
}
