...
public class CanopyNode extends AbstractTaskNode {

    private static final Logger LOGGER = LoggerFactory.getLogger(CanopyNode.class);
    ...
    public DataSet calculate(DataSet in, List<String> inRecordKeys, List<String> outRecordKeys) {
        ...
        LOGGER.info("[CANPOY] Start calculation, in= {}", in.toString());
        ...
        try {
            ...
            LOGGER.info("[CANPOY] vectors= {}", vectors);
            if (vectors.size() == inputKeys.size()) { // 将聚类数据写入临时文件
                ...
                LOGGER.error("[CANPOY] Feature value extraction from given data failed~!!");
            }
...
//            System.out.println("Preparing Input");
            LOGGER.info("[CANPOY] Preparing Input");
            InputDriver.runJob(input, directoryContainingConvertedInput, "org.apache.mahout.math.RandomAccessSparseVector");
//            System.out.println("Running Canopy to get initial clusters");
            LOGGER.info("[CANPOY] Running Canopy to get initial clusters");
            LOGGER.info("[CANPOY] Run Clustering: {}", runClustering);
            CanopyDriver.run(conf, directoryContainingConvertedInput, output, measure, t1, t2, runClustering, 0.0D, false);
            Path outGlob = new Path(output, "clusters-*-final");
            Path clusteredPoints = new Path(output, "clusteredPoints");
//            System.out.println("Dumping out clusters from clusters: " + outGlob + " and clusteredPoints:" + clusteredPoints);
            LOGGER.info("[CANPOY] Dumping out clusters from clusters: {} and clusteredPoints: {}", outGlob, clusteredPoints);
            ...
            LOGGER.info("[CANPOY] resultValue==== {}", resultSb.toString());
        ...
        LOGGER.info("[CANPOY] Returning calculation result");
        return out;
    }
