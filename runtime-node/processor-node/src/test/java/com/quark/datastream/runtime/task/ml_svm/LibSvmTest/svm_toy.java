...
public class svm_toy extends Applet {
	...
	void button_run_clicked(String args)
	{
			...
			param.svm_type == svm_parameter.NU_SVR)
		{
			...
			svm_model model = com.quark.datastream.runtime.task.ml_svm.LibSvmTest.svm.svm_train(prob, param);
			...
			for (int i = 0; i < XLEN; i++)
			{
				...
				j[i] = (int)(YLEN* com.quark.datastream.runtime.task.ml_svm.LibSvmTest.svm.svm_predict(model, x));
			}
		...
		else
		{
			...
			for (int i = 0; i < XLEN; i++)
				for (int j = 0; j < YLEN ; j++) {
					...
					double d = com.quark.datastream.runtime.task.ml_svm.LibSvmTest.svm.svm_predict(model, x);
